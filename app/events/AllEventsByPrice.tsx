...
import { Event, getAllEventsSortPriceInsecure } from '../../database/events';
import { getUserPublicByIdInsecure } from '../../database/users';

export default async function AllEventsByPrice() {
  const events = (await getAllEventsSortPriceInsecure()).map(
    async (event: Event) => {
